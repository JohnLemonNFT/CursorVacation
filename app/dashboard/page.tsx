  ...
  const fetchTrips = useCallback(
    async (force = false) => {
      ...
      try {
        ...
        if (tripsError) {
          throw new Error(handleSupabaseError(tripsError, "fetching user trips"))
        }
        ...
        if (memberError) {
          throw new Error(handleSupabaseError(memberError, "fetching member trips"))
        }
        ...
        if (memberTrips && memberTrips.length > 0) {
          ...
          if (error) {
            throw new Error(handleSupabaseError(error, "fetching additional trips"))
          }
